...
def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        log_event("ERROR", f"an error happend wile detecting language: {e}")
    ...
    try:   
        ...
        log_event("ERROR", f"an error occured while translating to english: {e}")
    
    try:
        response, score = chat_with_bot(prompt=user_input, faq_source=faq_source)
    except Exception as e:
        log_event("ERROR", f"an error occured while chating with bot: {e}")
    ...
    try:     
        ...
        log_event("ERROR", f"an error occured while translating to arabic: {e}")
